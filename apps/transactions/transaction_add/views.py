    ...
    def post(self, request):
        ...
        if form.is_valid():
            if not self.transaction_exists(form.cleaned_data):
                form.save()
                messages.success(request, 'تراکنش با موفقیت افزوده شد')
            else:
                messages.error(request, 'تراکنش با این مشخصات موجود است')
        else:
            messages.error(request, 'عملیات ناموفق / خطای نامشخص')
