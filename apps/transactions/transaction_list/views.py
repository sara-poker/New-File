...
class TransactionListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Convert transaction dates to Jalali (Shamsi) dates
        for transaction in transactions:
            transaction.transaction_date = datetime.fromgregorian(date=transaction.transaction_date).strftime("%Y-%m-%d")
            transaction.due_date = datetime.fromgregorian(date=transaction.due_date).strftime("%Y-%m-%d")
